...
def resolve_public_did(did: str) -> str:
    """
    Resolve a public DID using the universal resolver and extract the public key.
    Replace this with a specific DID method resolver if needed.
    """
    try:
        ...
        if response.status_code == 200:
            ...
            # This assumes the DID Document contains verificationMethod with publicKeyPem
...
class QRCodeDownloadView(APIView):
    """
    API view for downloading QR code for DID or credential
    """
    permission_classes = [AllowAny]
    
    def get(self, request, user_id=None, credential_id=None):
        # Check if we're getting a credential QR code
        if credential_id:
            # Get credential
            try:
                ...
                # For public credential QR downloads, don't restrict access
                # We want QR codes to be publicly accessible for verification purposes
                ...
                response['Content-Disposition'] = f'attachment; filename="credential_qr_{credential.id}.png"'
...
class VerifyCredentialView(APIView):
    ...
    def get(self, request):
        ...
        credential_id = request.query_params.get('credential_id') or request.query_params.get('id')
        if credential_id:
    ...
    def _verify_credential(self, credential_id, request=None):
        print(f"Attempting to verify credential: {credential_id}")
        ...
        if not credential_id.startswith('vc:'):
            credential_id = f"vc:{credential_id}"
        ...
        try:
            ...
            # Recreate credential data with exact format
            credential_data = {
                "type": credential.credential_type,
                "issuer": credential.issuer_did.did,
                "subject": credential.subject_did.did,
                "claims": credential.claims,
                "issuanceDate": credential.issuance_date.isoformat().split('+')[0]  # Remove timezone
            }
            
            print("Verifying credential data:", json.dumps(credential_data, sort_keys=True))  # Debug logging
        ...
        if user.is_authenticated:
            ...
            if hasattr(user, 'did'):
                ...
                if qr_code_base64.startswith('data:image/png;base64,'):
                    context['qr_code'] = qr_code_base64.split(',', 1)[1]
                else:
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            if is_authorized:
                ...
                # Format claims as pretty JSON for developers
                context['claims_json'] = json.dumps(credential.claims, indent=2)
                
                # Format claims as human-readable text
                human_readable_claims = []
                for key, value in credential.claims.items():
                ...
                context['human_readable_claims'] = human_readable_claims
